...
if [[ -t 1 || "${FORCE_COLOR:-0}" == 1 ]]; then
  ...
  C_RST=; C_B=; C_DIM=; C_RED=; C_GRN=; C_YLW=; C_BLU=; C_CYN=; C_MAG=; C_GRY=
fi
...
banner() {
  printf '%s%s╭──────────────────────────────────────────────────────────────╮%s\n' "$C_B" "$C_CYN" "$C_RST"
  printf '%s%s│%s   %sREMNAWAVE NODE%s  ·  VLESS · Trojan · Hysteria2 · Caddy L4     %s%s│%s\n' \
    "$C_B" "$C_CYN" "$C_RST" "$C_B$C_MAG" "$C_RST$C_GRY" "$C_B$C_CYN" "" "$C_RST"
  printf '%s%s╰──────────────────────────────────────────────────────────────╯%s\n' "$C_B" "$C_CYN" "$C_RST"
}
